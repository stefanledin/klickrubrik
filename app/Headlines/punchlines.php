<?php

namespace App\Headlines;

function getPunchlines() {
    return [
        '– du kan inte gissa vad som hände sen!',
        '– du anar inte vad som hände!',
        '– då hände det oväntade!',
        '– resultatet är chockerande!',
        '– ingen kunde ana följderna!',
        '– konsekvenserna var oanade!',
        '– ingen kunde förutse följderna!',
        '– det som hände har fått en hel värld att förundras!',
        '– det som hände har fått ett helt land att förundras!',
        '– det som hände har fått en hel läkarkår att förundras!',
        '– det som hände kommer förändra din syn på mänskligheten!',
        '– det som hände förändrade allt!',
        '– det som hände förändrade hennes liv!',
        '– det som hände förändrade hans liv!',
        '– det som sen hände fick mig att gråta!',
        '– resultatet har chockat en hel värld!',
    ];
}